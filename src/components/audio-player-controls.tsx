...
export function AudioPlayerControls({ audioUrl, disabled }: AudioPlayerControlsProps) {
  ...
  return (
    <div className={cn("border border-[#1a1a1a] bg-black p-6", disabled && "opacity-40")}>
      <div className="flex items-center gap-4">
        <button
          type="button"
          onClick={togglePlay}
          disabled={!audioUrl || disabled}
          className="flex h-14 w-14 items-center justify-center border border-[#1a1a1a] bg-[#0a0a0a] transition hover:border-white"
        >
          {isPlaying ? (
        ...
        <div className="flex-1 space-y-2">
          <input
            ...
            className="h-1 w-full cursor-pointer appearance-none bg-[#1a1a1a] accent-white [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:bg-white [&::-moz-range-thumb]:h-3 [&::-moz-range-thumb]:w-3 [&::-moz-range-thumb]:border-0 [&::-moz-range-thumb]:bg-white"
          />
        ...
        <button
          ...
          className={cn(
            "ml-4 flex h-10 w-10 items-center justify-center border border-[#1a1a1a] transition",
            loop ? "bg-white" : "bg-transparent",
          )}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" className={cn("stroke-current", loop ? "text-black" : "text-white")} fill="none" strokeWidth={1.6}>
